...
class VirialScaling(BaseValidationTest):
    ...
    def run_on_single_catalog(self, catalog_instance, catalog_name, output_dir):
        ...
        if not catalog_instance.has_quantities(
                ['halo_mass', 'halo_id', 'velocity_x', 'velocity_y', 'velocity_z', 'redshift']):
        ...
        for unique_id in unique_id_list:
            ...
            while unique_id == cut_id_list[index]:
                ...
                if(stellar_masses[index] > self.stellarcut):
                    vel_mag = np.sqrt(np.power(
                        vx_list[index], 2)+np.power(vy_list[index], 2) +
        ...
        if (self.c_axis == 'number'):
           img = self.summary_ax.scatter(
            x_axis, vel_dispersion[mask_num], c=galaxy_num_list[mask_num],
            norm=LogNorm(), label=catalog_name + ' cluster')

        elif(self.c_axis == 'redshift'):
           img = self.summary_ax.scatter(x_axis, vel_dispersion[mask_num],
                c=median_r[mask_num], norm=LogNorm(), label=catalog_name + ' cluster')
        #make plot
        x = np.linspace(smallest*.75, largest*1.5)
        self.summary_ax.plot(x, eval("1082*(x/10**15)**.3361"),
                c="red", label="Evrard et al. 2007")
        ...
        self.summary_ax.set_ylim(
            np.min(vel_dispersion[mask_num])*.3, np.max(vel_dispersion[mask_num])*5)
        self.summary_ax.set_xlim(smallest*.75, largest*1.5)
        self.summary_ax.set_yscale('log')
        self.summary_ax.set_xlabel(r'$Mh(z)  (M_{\odot})$')
